...
class ProjectController extends Controller
{
    ...
    public function destroy(Project $project, Request $request): RedirectResponse
    {
        ...
        $video = $project->video;
        $invoiceProof = $project->invoice_proof;
        $gallery = $project->gallery;

        try {
            $project->inquiries()->delete();
            $project->delete();
        } catch (\Throwable $e) {
            ...
            return back()->with('status', 'Unable to delete project right now. Please try again.');
        }

        $this->deleteFile($beforeImage);
        $this->deleteFile($afterImage);
        $this->deleteFile($video);
        $this->deleteFile($invoiceProof);
