...
<section class="section" style="padding-top: 0;">
    <div class="container client-form-wrap">
        <div class="form-card">
            <h2 class="client-form-title">Your inquiry conversations</h2>

            @if ($inquiries->count() === 0)
                <div class="placeholder-card">No inquiries yet. Message a designer from any project page.</div>
            @else
                <div style="display: grid; gap: 1rem;">
                    @foreach ($inquiries as $inquiry)
                        <article class="card" style="padding: 1rem;">
                            <div class="card-top">
                                <span class="chip">{{ $inquiry->project?->title ?? 'Project' }}</span>
                                <span class="card-meta">{{ optional($inquiry->created_at)->diffForHumans() }}</span>
                            </div>
                            <h3 style="margin: 0.4rem 0;">{{ $inquiry->project?->designer?->user?->name ?? 'Designer' }}</h3>

                            <div class="card" style="margin-top: 0.65rem; background: var(--surface-muted); border-style: dashed;">
                                <p style="margin: 0; font-weight: 600;">Your first message</p>
                            ...
                            @if ($inquiry->replies->isNotEmpty())
                                <div style="margin-top: 0.9rem; display: grid; gap: 0.6rem;">
                                    @foreach ($inquiry->replies as $reply)
                                        @php
                                            $isClientReply = (int) optional($reply->user)->id === (int) auth()->id();
                                        @endphp
                                        <div class="card" style="padding: 0.85rem; background: {{ $isClientReply ? 'var(--surface-muted)' : 'var(--surface)' }};">
                                            <div class="card-top" style="margin-bottom: 0.35rem;">
                                                <span class="chip" style="background: {{ $isClientReply ? 'var(--color-primary)' : 'var(--line)' }}; color: {{ $isClientReply ? 'white' : 'inherit' }};">
                                                    {{ $isClientReply ? 'Your reply' : 'Designer reply' }}
                                                </span>
                            ...
                            <form method="post" action="{{ route('client.inquiries.reply', $inquiry) }}" style="margin-top: 0.9rem; display: grid; gap: 0.6rem;">
                                @csrf
                                <label class="field" style="margin: 0;">
                                    <span style="font-weight: 600;">Send reply</span>
                                    <textarea name="message" rows="3" maxlength="2000" placeholder="Write your response to the designer..."></textarea>
                                </label>
                                @error('message')
                                    <span class="field-error">{{ $message }}</span>
                                @enderror
                ...
                @if (method_exists($inquiries, 'links'))
                    <div class="pagination" style="margin-top: 1rem;">{{ $inquiries->links('partials.pagination') }}</div>
                @endif
            @endif
        </div>
    </div>
